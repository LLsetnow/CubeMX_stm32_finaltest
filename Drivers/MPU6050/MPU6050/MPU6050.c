...
void get_angle(void)
{
	...
	gyro.x = (gx - gyro_zero.x) / 16.4;
	gyro.y = (gy - gyro_zero.y) / 16.4;
	gyro.z = (gz - gyro_zero.z) / 16.4;
	...
	pitch.acc 			= atan2(acc.x, acc.z) * 180 / 3.14; 
	pitch.gyro 			= pitch.angle_last + gyro.y;
	pitch.angle 		= K * pitch.acc + (1 - K) * pitch.gyro;
	pitch.angle_last 	= pitch.angle;
	
	roll.acc 			= atan2(acc.y, acc.z)* 180 / 3.14; 
	roll.gyro 			= roll.angle_last + gyro.x;
	roll.angle 			= K * roll.acc + (1 - K) * roll.gyro;
	roll.angle_last 	= roll.angle;
